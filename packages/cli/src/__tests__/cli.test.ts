...
describe("COMPONENT_REGISTRY", () => {
  ...
  it("each entry has required fields", () => {
    for (const entry of COMPONENT_REGISTRY) {
      ...
      expect(entry.npmDependencies).toBeDefined();
  ...
  it("npm dependencies are well-formed", () => {
    for (const entry of COMPONENT_REGISTRY) {
      for (const [pkg, ver] of Object.entries(entry.npmDependencies)) {
...
describe("listComponents", () => {
  ...
  it("filters by category", () => {
    ...
    expect(customerComponents.every((c) => c.category === "customer")).toBe(true);
  });
