---
title: EmbedConfigurator
description: Admin tool for generating customizable embed code snippets.
---

Generates ready-to-paste HTML snippets for embedding the SlotKit booking flow in three modes: inline, popup, and floating button. Allows customization of event type, primary color, and supports easy copy-to-clipboard functionality.

## Install

```bash
npx thebookingkit add embed-configurator
```

## Usage

```tsx
import { EmbedConfigurator, type EmbedEventTypeOption } from "@/components/embed-configurator";

export function AdminEmbedSettings() {
  const eventTypes: EmbedEventTypeOption[] = [
    { slug: "haircut", title: "Haircut" },
    { slug: "color", title: "Color Treatment" },
    { slug: "extensions", title: "Hair Extensions" },
  ];

  return (
    <div className="admin-panel">
      <h1>Embed Your Booking Flow</h1>
      <EmbedConfigurator
