---
title: KioskSettingsPanel
description: Slide-out settings panel for configuring kiosk display, color coding, visible fields, and auto-lock timeout.
---

Slide-out settings panel that allows administrators and reception staff to customize how the kiosk displays calendar events. Configure visible fields, block density, color coding, working hours, and security settings like auto-lock timeout.

## Install

```bash
npx thebookingkit add kiosk-settings-panel
```

## Usage

```tsx
import { KioskSettingsPanel, type KioskSettingsValues } from "@/components/kiosk-settings-panel";
import { useState } from "react";

const defaultSettings: KioskSettingsValues = {
  defaultView: "day",
...
export default function KioskDashboard() {
  ...
  const handleSaveSettings = async (newSettings: KioskSettingsValues) => {
...
export interface KioskSettingsValues {
  defaultView: "day" | "3day" | "week";
  blockDensity: "compact" | "standard" | "detailed";
  colorCoding: "status" | "event_type" | "source";
  dayStartHour: number;
  dayEndHour: number;
